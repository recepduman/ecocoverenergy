...
function getTestExecutionDuration(paths) {
    $.ajax({
        type: "GET",
        url: '/getTestExecutionDuration',
        dataType: "JSON",
        data: ({paths}),
        beforeSend: function () {
        ...
        success: function (data) {
            $('#test-execution-duration').text(toHHMMSS(data[0].DURATION));
        }
    });
}
...
$('#btn-generate-report').on('click', function() {
    ...
    getTestExecutionDuration(allPaths);
...
function toHHMMSS (secondsForFormatting) {
    var sec_num = parseInt(secondsForFormatting, 10); // don't forget the second param
